...
//16-11-2024//
  ...
  // التعامل مع النقر على التبويبات
  $('.widget-nav-tabs .nav-link').on('click', function (e) {
    e.preventDefault(); // منع الإجراء الافتراضي للرابط
    const tab = $(this).data('tab'); // الحصول على قيمة data-tab
    const newUrl = `data-url-${tab}.json`; // تحديد الرابط الجديد بناءً على التبويب

    // إزالة الـ active من التبويبات الأخرى
    $('.widget-nav-tabs .nav-link').removeClass('active');

    // إضافة الـ active إلى التبويب الحالي
    $(this).addClass('active');

    // تحديث الرابط وإعادة تحميل البيانات
